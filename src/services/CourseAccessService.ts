...
class CourseAccessService {
    ...
    getTotalCourseAccess(courseId: number) {
        return this.promisePool.query("SELECT COUNT(*) FROM `CourseAccess` WHERE accessed > DATE_SUB(NOW(),INTERVAL 7 DAYS)\
        AND `c_id` =?", [courseId]);
    }

    // returns total number of users who accessed the course during the last seven days
    getCourseAccessByUsers(courseId: number) {
        return this.promisePool.query("SELECT COUNT(*) FROM `CourseUserRelation` WHERE visited > DATE_SUB(NOW(),INTERVAL 7 DAYS)\
        AND `c_id` =?", [courseId]);
    }
}
